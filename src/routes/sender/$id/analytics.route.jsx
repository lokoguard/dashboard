...
export default function Index() {
  ...
  const [diskUsedPercentageStatSeries, setDiskUsedPercentageStatSeries] =
    useState([]);
  ...
  const updateMemoryStats = async (rawSeries) => {
    ...
    for (let i = 0; i < rawSeries.length; i++) {
      seriesMap["available"].push([
        rawSeries[i].timestamp,
        rawSeries[i].memStat.available_gb,
      ]);
      seriesMap["cached"].push([
        rawSeries[i].timestamp,
        rawSeries[i].memStat.cached_gb,
      ]);
      seriesMap["free"].push([
        rawSeries[i].timestamp,
        rawSeries[i].memStat.free_gb,
      ]);
      seriesMap["total"].push([
        rawSeries[i].timestamp,
        rawSeries[i].memStat.total_gb,
      ]);
      seriesMap["used"].push([
        rawSeries[i].timestamp,
        rawSeries[i].memStat.used_gb,
      ]);
    }
  ...
  const updateTemperatureStats = async (rawSeries) => {
    ...
    for (let i = 0; i < rawSeries.length; i++) {
      for (let j in rawSeries[i].temperatureStats) {
        seriesMap[rawSeries[i].temperatureStats[j].sensor].push([
          rawSeries[i].timestamp,
          rawSeries[i].temperatureStats[j].temperature_celcius,
        ]);
      }
    }
    // create series
  ...
  const updateDiskStats = async (rawSeries) => {
    ...
    for (let i = 0; i < rawSeries.length; i++) {
      for (let recordIndex in rawSeries[i].diskStats) {
        ...
        seriesMap[record.path].push([
          rawSeries[i].timestamp,
          record.used_percent,
        ]);
        seriesFreeGbMap[record.path].push([
          rawSeries[i].timestamp,
          record.free_gb,
        ]);
      }
  ...
  return (
    <>
      <SimpleGrid columns={2} spacing={10} maxH="80vh" overflow="auto">
        ...
        {/* Temperature stats */}
        {areaChart(
          temperatureStatSeries,
          "Temperature Stats (°C)",
          screenWidth,
        )}
        {/*  Disk stats (Used Percentage)  */}
        {areaChart(
          diskUsedPercentageStatSeries,
          "Disk Stats (Used Percentage)",
          screenWidth,
        )}
        {/*  Disk stats (Free GB)  */}
